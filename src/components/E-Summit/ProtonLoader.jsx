...
// ─── TIMELINE (ms) ────────────────────────────────────────────────────────────
const T = {
  PROTON_TRAVEL : 3000,   // proton glides in
  COLLISION     : 3000,   // collision at 3s
  EXPLOSION     : 3600,   // 0.6s flash
  GATHER        : 5800,   // 2.2s burst + gather
  HOLD          : 7200,   // 1.4s rocket assembled
  LAUNCH        : 7600,   // 0.4s ignition
  TEXT          : 9000,   // text burns in
  DONE          : 12000,  // done
};
...
export default function ProtonLoader({ onComplete }) {
  ...
  useEffect(() => {
    ...
    function getRocketScale() { return Math.min(canvas.height * 0.12, 70); }

    function getRocketShape(rs) {
      return [
        { x: 0, y: -1.0 }, { x: 0.2, y: -0.68 }, { x: 0.24, y: -0.18 },
        { x: 0.52, y: 0.55 }, { x: 0.3, y: 0.44 }, { x: 0.24, y: 1.0 },
        { x: 0.16, y: 1.06 }, { x: -0.16, y: 1.06 },
        { x: -0.24, y: 1.0 }, { x: -0.3, y: 0.44 }, { x: -0.52, y: 0.55 },
        { x: -0.24, y: -0.18 }, { x: -0.2, y: -0.68 },
    ...
    function getRocketTargets(cx, cy, shape) {
      const pts = []; const N = shape.length;
      for (let i = 0; i < TARGET_COUNT; i++) {
        const t = i / TARGET_COUNT;
        const seg = t * N; const si = Math.floor(seg) % N; const sf = seg - Math.floor(seg);
        const a = shape[si], b = shape[(si + 1) % N];
        pts.push({
    ...
    function initState() {
      ...
      for (let i = 0; i < 90; i++) {
        ...
        expParticles.push({
          ...
          r: 1 + Math.random() * 2.5,
          hue: 190 + Math.random() * 80, alpha: 1, target: null,
        });
      }

      stateRef.current = {
        W, H, CX, CY, rs, shape, targets, groundY,
        electronAngles: ELECTRONS.map(e => e.phase),
        protonX: -50, protonY: CY + 15,
        expParticles,
        rocketY: 0,
        exhaustParts: [],
        groundLetters: [],
    ...
    function drawRocket(cx, cy, alpha) {
      ...
      ctx.fillStyle = aura; ctx.beginPath(); ctx.arc(0, 0, rs * 1.2, 0, Math.PI * 2); ctx.fill();

      // nose cone
      ctx.beginPath();
      ctx.moveTo(0, -NOSE);
      ctx.bezierCurveTo(BW * .45, -NOSE * .5, BW, -NOSE * .05, BW, BH * .06);
      ctx.lineTo(-BW, BH * .06);
      ctx.bezierCurveTo(-BW, -NOSE * .05, -BW * .45, -NOSE * .5, 0, -NOSE);
      let ng = ctx.createLinearGradient(-BW, -NOSE, BW, -NOSE * .1);
      ng.addColorStop(0, '#8ab8d8'); ng.addColorStop(.38, '#ddeeff');
      ng.addColorStop(.65, '#b8d4ea'); ng.addColorStop(1, '#6090b0');
      ctx.fillStyle = ng; ctx.fill();
      ctx.strokeStyle = 'rgba(180,215,240,.55)'; ctx.lineWidth = .9; ctx.stroke();

      // main body
      let bg = ctx.createLinearGradient(-BW, 0, BW, 0);
      bg.addColorStop(0, '#3a5570'); bg.addColorStop(.1, '#7aaabe');
      bg.addColorStop(.32, '#c8e2f2'); bg.addColorStop(.5, '#ddeeff');
      bg.addColorStop(.7, '#9abfd8'); bg.addColorStop(.9, '#527090');
      bg.addColorStop(1, '#2e4460');
      ctx.fillStyle = bg; ctx.fillRect(-BW, BH * .06, BW * 2, BH * .88);
      ctx.strokeStyle = 'rgba(140,190,220,.3)'; ctx.lineWidth = .5;
      ctx.strokeRect(-BW, BH * .06, BW * 2, BH * .88);

      // panel ribbing
      for (let i = 1; i <= 5; i++) {
        const py = BH * .06 + i * (BH * .88 / 6);
        ctx.beginPath(); ctx.moveTo(-BW, py); ctx.lineTo(BW, py);
        ctx.strokeStyle = `rgba(90,150,190,${.1 + i * .018})`; ctx.lineWidth = .5; ctx.stroke();
      }
      // sheen
      ctx.beginPath(); ctx.rect(-BW * .07, -NOSE, BW * .12, NOSE + BH * .94);
      let sh = ctx.createLinearGradient(-BW * .07, 0, BW * .05, 0);
      sh.addColorStop(0, 'rgba(255,255,255,0)');
      sh.addColorStop(.5, 'rgba(255,255,255,.12)');
      sh.addColorStop(1, 'rgba(255,255,255,0)');
      ctx.fillStyle = sh; ctx.fill();

      // logo band
      const bandY = BH * .38, bandH = BH * .22;
      ctx.fillStyle = 'rgba(20,60,160,.55)';
      ctx.fillRect(-BW, bandY, BW * 2, bandH);
      ctx.save();
      ctx.font = `bold ${BW * .45}px 'Courier New',monospace`;
      ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
      ctx.shadowBlur = 7; ctx.shadowColor = 'rgba(80,180,255,.8)';
      ctx.fillStyle = 'rgba(190,230,255,.92)';
      ctx.fillText("E-SUMMIT'26", 0, bandY + bandH * .5);
      ctx.shadowBlur = 0; ctx.restore();

      // porthole windows
      [{ cy2: -NOSE * .32 }, { cy2: -NOSE * .02 }].forEach(w => {
        ctx.beginPath(); ctx.arc(0, w.cy2, BW * .19, 0, Math.PI * 2);
        ctx.fillStyle = '#1e2e40'; ctx.fill();
        ctx.strokeStyle = 'rgba(140,195,230,.5)'; ctx.lineWidth = 1; ctx.stroke();
        let wg = ctx.createRadialGradient(-BW * .06, w.cy2 - BW * .06, 0, 0, w.cy2, BW * .17);
        wg.addColorStop(0, 'rgba(190,235,255,.92)');
        wg.addColorStop(.4, 'rgba(70,150,255,.65)');
        wg.addColorStop(1, 'rgba(10,35,90,.85)');
        ctx.beginPath(); ctx.arc(0, w.cy2, BW * .17, 0, Math.PI * 2);
        ctx.fillStyle = wg; ctx.fill();
        ctx.beginPath(); ctx.arc(-BW * .06, w.cy2 - BW * .07, BW * .05, 0, Math.PI * 2);
      ...
      const SKY = BH * .94;
      ctx.beginPath();
      ctx.moveTo(-BW, SKY); ctx.lineTo(-BW * 1.18, BH + rs * .04);
      ctx.lineTo(BW * 1.18, BH + rs * .04); ctx.lineTo(BW, SKY);
      ctx.closePath();
      let sk = ctx.createLinearGradient(-BW * 1.18, 0, BW * 1.18, 0);
      sk.addColorStop(0, '#2e3e52'); sk.addColorStop(.35, '#506a80');
      sk.addColorStop(.65, '#607888'); sk.addColorStop(1, '#2e3e52');
      ctx.fillStyle = sk; ctx.fill();
      ctx.strokeStyle = 'rgba(90,140,180,.35)'; ctx.lineWidth = .6; ctx.stroke();

      // nozzle bell
      const NZ = BH + rs * .04;
      ctx.beginPath();
      ctx.moveTo(-BW * .36, NZ);
      ctx.bezierCurveTo(-BW * .36, NZ + rs * .04, -BW * .54, NZ + rs * .1, -BW * .56, NZ + rs * .15);
      ctx.lineTo(BW * .56, NZ + rs * .15);
      ctx.bezierCurveTo(BW * .54, NZ + rs * .1, BW * .36, NZ + rs * .04, BW * .36, NZ);
      ctx.closePath();
      let nz = ctx.createLinearGradient(-BW * .56, 0, BW * .56, 0);
      nz.addColorStop(0, '#1a2230'); nz.addColorStop(.3, '#445868');
      nz.addColorStop(.5, '#607080'); nz.addColorStop(.7, '#445868'); nz.addColorStop(1, '#1a2230');
      ctx.fillStyle = nz; ctx.fill();
      ctx.strokeStyle = 'rgba(70,120,155,.45)'; ctx.lineWidth = .6; ctx.stroke();
      ctx.beginPath(); ctx.ellipse(0, NZ + rs * .14, BW * .34, BW * .07, 0, 0, Math.PI * 2);
      ctx.fillStyle = 'rgba(4,8,14,.95)'; ctx.fill();

      // grid fins x4
      [[-1, -.75], [1, -.75], [-1, .3], [1, .3]].forEach(([sx, sy]) => {
        const fx = BW * (sx > 0 ? 1 : -1), fy = BH * sy;
        const fw = BW * .34, fh = BH * .2;
        ctx.save(); ctx.translate(fx, fy);
        ctx.beginPath(); ctx.rect(sx > 0 ? 0 : -fw, -fh * .5, fw, fh);
        let fg = ctx.createLinearGradient(sx > 0 ? 0 : -fw, 0, sx > 0 ? fw : 0, 0);
        fg.addColorStop(0, sx > 0 ? 'rgba(42,60,80,.9)' : 'rgba(72,100,128,.9)');
        fg.addColorStop(1, sx > 0 ? 'rgba(72,100,128,.9)' : 'rgba(42,60,80,.9)');
        ctx.fillStyle = fg; ctx.fill();
        ctx.strokeStyle = 'rgba(90,140,180,.3)'; ctx.lineWidth = .55; ctx.stroke();
        for (let gi = 1; gi < 3; gi++) {
          const gx = sx > 0 ? gi * (fw / 3) : -fw + gi * (fw / 3);
          ctx.beginPath(); ctx.moveTo(gx, -fh * .5); ctx.lineTo(gx, fh * .5);
          ctx.strokeStyle = 'rgba(80,130,170,.22)'; ctx.lineWidth = .4; ctx.stroke();
        }
        ctx.restore();
      });

      // antenna
      ctx.strokeStyle = 'rgba(180,210,240,.6)'; ctx.lineWidth = .9;
      ctx.beginPath(); ctx.moveTo(0, -NOSE); ctx.lineTo(0, -NOSE - rs * .12); ctx.stroke();
      ctx.beginPath(); ctx.arc(0, -NOSE - rs * .12, 2.2, 0, Math.PI * 2);
      ctx.fillStyle = 'rgba(200,230,255,.85)';
      ctx.shadowBlur = 5; ctx.shadowColor = 'rgba(120,200,255,.7)';
    ...
    function frame(now) {
      ...
      bg.addColorStop(0, "#010408"); bg.addColorStop(0.65, "#040b18"); bg.addColorStop(1, "#080315");
      ...
      if (el >= T.HOLD) {
        ...
        s.exhaustParts.forEach(p => {
          p.x += p.vx; p.y += p.vy; p.r *= 0.968; p.life -= 0.02;
        ...
        if (el >= T.TEXT) {
          ...
          [1, 0.65, 0.35].forEach((f, i) => {
            ctx.beginPath(); ctx.arc(CX, groundY, rs2 * f, Math.PI, Math.PI * 2);
